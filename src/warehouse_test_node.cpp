#include "babyk_drone_manager/warehouse_test_node.h"

using namespace std::chrono_literals;

WarehouseTestNode::WarehouseTestNode() 
    : Node("warehouse_test_node"),
      ...
      command_dist_(0, 99)
{
    ...
    this->declare_parameter("lateral_randomness", 0.3);      // Randomness for lateral offset
    this->declare_parameter("random_dir_prob", 0.1);           // 10% chance to explore in a random direction
    this->declare_parameter("random_node_prob", 0.1);          // 10% chance to pick a random known free node
    ...
    lateral_randomness_ = this->get_parameter("lateral_randomness").as_double();
    random_dir_prob_ = this->get_parameter("random_dir_prob").as_double();
    random_node_prob_ = this->get_parameter("random_node_prob").as_double();
    ...
    status_subscriber_ = this->create_subscription<std_msgs::msg::String>(
        ...
        std::bind(&WarehouseTestNode::status_callback, this, std::placeholders::_1));
    ...
    octomap_subscriber_ = this->create_subscription<octomap_msgs::msg::Octomap>(
        ...
        std::bind(&WarehouseTestNode::octomap_callback, this, std::placeholders::_1));
        
    odometry_subscriber_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/px4/odometry/out", sensor_qos,
        std::bind(&WarehouseTestNode::odometry_callback, this, std::placeholders::_1));
    ...
    command_timer_ = this->create_wall_timer(
        5s, std::bind(&WarehouseTestNode::command_timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "Warehouse Test Node initialized");
...
void WarehouseTestNode::status_callback(const std_msgs::msg::String::SharedPtr msg)
{
...
void WarehouseTestNode::octomap_callback(const octomap_msgs::msg::Octomap::SharedPtr msg)
{
...
void WarehouseTestNode::odometry_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
...
std::optional<Eigen::Vector3d> WarehouseTestNode::find_frontier_goal(bool& should_return)
{
    should_return = false;
    if (!octree_) {
    ...
    bool explore_random_dir = (prob_dist(gen_) < random_dir_prob_);
    
    if (explore_random_dir) {
        std::uniform_real_distribution<double> angle_dist(-M_PI, M_PI);
        original_yaw = angle_dist(gen_); 
        RCLCPP_INFO(this->get_logger(), "🔄 Reached goal! Exploring in a RANDOM DIRECTION (yaw=%.2f) this time!", original_yaw);
    }
    ...
    bool explore_random_node = (prob_dist(gen_) < random_node_prob_);

    if (explore_random_node && free_nodes > 0) {
        RCLCPP_INFO(this->get_logger(), "🎲 Picking a RANDOM KNOWN NODE from %d free nodes!", free_nodes);
        
        std::vector<Eigen::Vector3d> valid_nodes;
        std::vector<Eigen::Vector3d> occupied_nodes;
        ...
        for (octomap::OcTree::leaf_iterator it = octree_->begin_leafs(), end = octree_->end_leafs(); it != end; ++it) {
            ...
            if (octree_->isNodeOccupied(*it)) {
                // Collect obstacles that are somewhat relevant to flight altitude
                if (z > 0.0 && z < 3.0) {
                    occupied_nodes.push_back(Eigen::Vector3d(x, y, z));
                }
            } else {
                double dx = x - drone_x;
                double dy = y - drone_y;
                
                // Must be at a safe height and far enough away (at least 3 meters)
                if (z > 0.5 && z < 2.0 && (dx*dx + dy*dy > 9.0)) {
                    valid_nodes.push_back(Eigen::Vector3d(x, y, z));
                }
            }
        }
        
        if (!valid_nodes.empty()) {
            std::uniform_int_distribution<size_t> node_dist(0, valid_nodes.size() - 1);
            
            // Sample candidates and pick the one furthest from any obstacle, 
            // ensuring it is in a dense known area (no unknown/unmapped space around it)
            int max_attempts = 200;
            int candidates_evaluated = 0;
            Eigen::Vector3d best_node = valid_nodes[0];
            double max_clearance = -1.0;
            
            for (int i = 0; i < max_attempts && candidates_evaluated < 30; i++) {
                Eigen::Vector3d candidate = valid_nodes[node_dist(gen_)];
                
                // Density check: must be surrounded by mostly known space (radius 0.5m)
                // This prevents picking isolated noise voxels outside the map boundaries,
                // without being so strict that it rejects valid corridors.
                bool is_dense = true;
                int unknown_count = 0;
                int total_count = 0;
                double r = 0.5;
                for (double dx = -r; dx <= r; dx += 0.5) {
                    for (double dy = -r; dy <= r; dy += 0.5) {
                        total_count++;
                        octomap::OcTreeNode* n = octree_->search(candidate.x() + dx, candidate.y() + dy, candidate.z());
                        if (n == nullptr) { 
                            unknown_count++;
                        } else if (octree_->isNodeOccupied(n)) { 
                            is_dense = false;
                            break;
                        }
                    }
                ...
                // Skip if it touches a wall, or if more than half the surrounding space is unknown (likely outside map)
                if (!is_dense || (double)unknown_count / total_count > 0.5) continue; 
                
                candidates_evaluated++;
                
                double min_dist_to_obs = 10000.0;
                if (!occupied_nodes.empty()) {
                    for (const auto& obs : occupied_nodes) {
                        double dist_sq = (candidate - obs).squaredNorm();
                        if (dist_sq < min_dist_to_obs) {
                            min_dist_to_obs = dist_sq;
                        }
                    }
                }
                
                if (min_dist_to_obs > max_clearance) {
                    max_clearance = min_dist_to_obs;
                    best_node = candidate;
                }
            }
            ...
            if (candidates_evaluated > 0) {
                ...
                goal_in_octomap.pose.position.x = best_node.x();
                goal_in_octomap.pose.position.y = best_node.y();
                goal_in_octomap.pose.position.z = best_node.z();
                goal_in_octomap.pose.orientation.w = 1.0;
                
                try {
                    geometry_msgs::msg::PoseStamped goal_in_map = tf_buffer_->transform(goal_in_octomap, "map", tf2::durationFromSec(0.1));
                    RCLCPP_INFO(this->get_logger(), "🎯 DENSE RANDOM NODE FOUND! Octomap coords: [%.2f, %.2f, %.2f] (Clearance: %.2fm)",
                                best_node.x(), best_node.y(), best_node.z(), std::sqrt(max_clearance));
                    ...
                    // Fall through to standard exploration if TF fails
                }
            } else {
                RCLCPP_WARN(this->get_logger(), "Wanted to pick random node, but could not find any strictly DENSE node. Falling back to frontier.");
            }
        } else {
            RCLCPP_WARN(this->get_logger(), "Wanted to pick random node, but no valid node is far enough. Falling back to normal frontier.");
        }
    }

    double best_max_dist = -1.0;
    double best_yaw = original_yaw;
    bool any_found = false;
    double tunnel_width = 25.0; // Increased to 25.0m to allow wide zig-zag in the warehouse
    
    std::uniform_real_distribution<double> angle_dist(-M_PI, M_PI);
    
    // Try the current direction first. If blocked, try up to 10 random directions.
    for (int attempt = 0; attempt < 10; ++attempt) {
        double yaw = (attempt == 0) ? original_yaw : angle_dist(gen_);
        ...
        // Pass 1: Find the absolute furthest reach (max_dist_forward) along this heading
        for (octomap::OcTree::leaf_iterator it = octree_->begin_leafs(), end = octree_->end_leafs(); it != end; ++it) {
            if (!octree_->isNodeOccupied(*it)) {
                double x = it.getX();
                double y = it.getY();
        ...
        if (found && max_dist_forward > best_max_dist) {
            best_max_dist = max_dist_forward;
            best_yaw = yaw;
            any_found = true;
        }
        
        // If we found a direction with enough free space, stop searching!
        if (found && max_dist_forward >= min_forward_space_) {
            if (attempt > 0) {
                RCLCPP_INFO(this->get_logger(), "🚧 Obstacle detected ahead! Changing direction to yaw=%.2f", yaw);
            }
            break;
        }
    }

    if (any_found && best_max_dist >= min_forward_space_) { 
        double forward_x = std::cos(best_yaw);
        double forward_y = std::sin(best_yaw);
        ...
        for (octomap::OcTree::leaf_iterator it = octree_->begin_leafs(), end = octree_->end_leafs(); it != end; ++it) {
            if (!octree_->isNodeOccupied(*it)) {
                ...
                if (forward_dist >= best_max_dist - 3.0 && forward_dist <= best_max_dist && std::abs(lateral_pos) <= tunnel_width) {
        ...
        if (count > 0) {
            ...
            double lateral_span = (max_lateral - min_lateral) / 2.0;
            
            // Use lateral_randomness_ as a fraction of the available free space 
            // (0.0 = always go straight, 1.0 = use entire width of the room).
            // We cap it at 0.9 to avoid putting the goal exactly on the wall.
            double max_fraction = std::min(lateral_randomness_, 0.9);
            
            std::uniform_real_distribution<double> lat_dist(-lateral_span * max_fraction, lateral_span * max_fraction);
            double target_lateral = avg_lateral + lat_dist(gen_);
            ...
            double target_forward = best_max_dist * goal_distance_ratio_;
            
            double goal_x = drone_x + target_forward * forward_x + target_lateral * (-forward_y);
            double goal_y = drone_y + target_forward * forward_y + target_lateral * forward_x;
        ...
        if (!any_found) {
            RCLCPP_WARN(this->get_logger(), "Octomap has nodes, but NO free space was found in any direction!");
            should_return = true;
        } else {
            RCLCPP_WARN(this->get_logger(), "Octomap found free space, but max distance is %.2fm (must be > %.2fm)! Triggering return sequence.", best_max_dist, min_forward_space_);
            should_return = true;
        }
    }
...
void WarehouseTestNode::send_explore_flyto()
{
    geometry_msgs::msg::TransformStamped map_to_base;
    try {
        map_to_base = tf_buffer_->lookupTransform("map", "base_link", rclcpp::Time(0));
    ...
    if (last_assigned_goal_.has_value()) {
        Eigen::Vector3d goal = *last_assigned_goal_;
        double dx = goal(0) - map_to_base.transform.translation.x;
        double dy = goal(1) - map_to_base.transform.translation.y;
        double distance = std::sqrt(dx*dx + dy*dy);
        if (distance > 5.0) {
            RCLCPP_INFO(this->get_logger(), "Last goal is still %.2fm away. Continuing to follow it.", distance);
            std::string frame_name = "exploration_goal_" + std::to_string(exploration_goal_counter_);
            send_command("flyto(" + frame_name + ")");
            return;
        } else {
            RCLCPP_INFO(this->get_logger(), "Reached previous goal! Calculating new frontier.");
            last_assigned_goal_ = std::nullopt;
        }
    }
...
void WarehouseTestNode::send_command(const std::string& command)
{
...
void WarehouseTestNode::send_takeoff()
{
    send_command("takeoff");
}

void WarehouseTestNode::send_land()
{
    send_command("land");
}

void WarehouseTestNode::send_random_flyto()
{
...
bool WarehouseTestNode::is_system_idle()
{
...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<WarehouseTestNode>();
    
    RCLCPP_INFO(node->get_logger(), "Starting Warehouse Test Node");
